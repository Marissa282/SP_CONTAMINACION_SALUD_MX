...
## 6. Modelos no paramétricos para mejorar BIC y AIC 
*✏️ Investiguen si incluir modelos no paramétricos les ayuda a mejorar el BIC y el AIC de su mejor modelo.*

Se espera que los modelos no paramétricos, como los GAMs que se implementan con las librerías mgcv y gratia, mejoren indicadores como el AIC y el BIC porque permiten mayor flexibilidad en la forma de la relación entre las variables. A diferencia de un modelo lineal, que asume que la relación es recta y constante en toda la muestra, un GAM puede incorporar suavizadores que capturan curvaturas o cambios en la pendiente de manera automática. Esto significa que el modelo tiene más capacidad para reflejar la realidad de los datos sin necesidad de imponer una forma rígida desde el inicio. Por eso, al anticipar que las relaciones no siempre serán lineales, es razonable esperar que un GAM logre un mejor ajuste general y, con ello, ofrezca valores de AIC y BIC más bajos.
...
```{r}
mod_gam_O = gam(O~s(M), data = data, method = "REML") 
mod_gam_D = gam(D ~s(M), data = data, method = "REML")
mod_gam_Am = gam(Am~ s(M), data = data, method = "REML")
mod_gam_AL = gam(AL~s(E), data = data, method = "REML")
mod_gam_PCR = gam(PCR~ s(AL) + s(E), data = data, method = "REML")
mod_gam_T = gam(T ~s(PCR), data = data, method = "REML")
mod_gam_VD = gam(VD ~s(PCR) + s(E), data = data, method = "REML")
mod_gam_C = gam(C~ s(E), data = data, method = "REML")
mod_gam_H = gam(H~s(E), data = data, method = "REML")
mod_gam_I = gam(PCR~ s(E) , data = data, method = "REML")
```

```{r}
mod_M = lm(M ~ 1, data = data) 
mod_E = lm(E ~ 1, data = data)
```

```{r}
-1/2* (BIC(mod_gam_O) + BIC (mod_gam_D) + BIC (mod_gam_Am) + BIC (mod_gam_AL) + BIC (mod_gam_PCR) + BIC (mod_gam_T) +BIC(mod_gam_VD)+BIC(mod_gam_C)+ BIC (mod_gam_H) +BIC(mod_gam_I)+ BIC (mod_M)+BIC(mod_E))
```

En efecto, al estimar los modelos se observó que el BIC mejoró de manera clara al pasar de los modelos lineales a los no paramétricos. Esto confirma la expectativa inicial: los suavizadores incluidos en los GAMs lograron capturar mejor la variabilidad de los datos sin aumentar en exceso la complejidad, y por ello los criterios de información resultaron más favorables.
